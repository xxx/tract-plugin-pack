//! Profiling harness for the Huovilainen Moog `Ladder` effect.
//!
//! Mirrors `reverb_profile`: drives the per-sample hot path with no plugin
//! or GUI plumbing so `perf record` only sees DSP code.
//!
//! Build:
//!     cargo xtask native build --profile profiling --example ladder_profile -p multosis
//!
//! Profile:
//!     perf record -F 999 -g --call-graph dwarf -- target/profiling/examples/ladder_profile
...
fn run_one(label: &str, cutoff: f32, resonance: f32, drive_db: f32) {
    let mut effect = EffectInstance::new(EffectKind::Ladder);
    effect.set_sample_rate(SAMPLE_RATE);
    let defaults = default_params_for_kind(EffectKind::Ladder);
    let n_params = effect.parameters().len();
    for (i, &v) in defaults.iter().enumerate().take(n_params) {
        effect.set_param(i, v);
    }
    // Order: Cutoff (0), Resonance (1), Drive (2).
...
fn main() {
    println!("Ladder profile -- {SECONDS}s of audio per scenario at {SAMPLE_RATE} Hz");
    println!();
