...
dpMeter5 is the standard meter for clip-to-zero, but it has practical limits at scale:

| | GS Meter (×50) | dpMeter5 (×50) |
|---|---|---|
| **CPU** | 16% | 72% |
| **Memory** | 48 MB | 1.3 GB |

GS Meter achieves this through:
...
- **Single-process hosting** — all instances share one host process, unlike dpMeter5 which runs through Wine/yabridge with per-instance overhead.
...
2. Set your reference level (e.g., -14 dB for streaming, -18 dB for broadcast)
3. Play a representative section of the song
4. Click the "→ Gain" button next to RMS (Int) to auto-set the gain
5. Verify the peak and true peak are within acceptable limits
